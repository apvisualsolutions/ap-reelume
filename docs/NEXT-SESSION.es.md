...
4. ~~**La puerta de escalares**~~ — **hecha el 2026-08-19**: [la evidencia](evidence/stable/audit-scalar-gate.md). Tal como estaba decidida, y con dos cosas que la decisión no tenía: los `MotionDuration*` los vigilaban **dos** pruebas, no una, y su garantía **se mudó** a `FluentThemeService.MotionDuration` en vez de perderse. Probada fallando en **tres** direcciones. **La puerta de escalares cuenta el consumo en CUALQUIER `.axaml` de `src/`**, incluido el propio
