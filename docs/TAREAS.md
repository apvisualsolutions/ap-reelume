...
| `ENG-021` | 2026-09-13 | `ABIERTA` | **Subir la gamma llena la imagen de cuadros de tonos planos**, así que el mando que existe para mejorar la imagen la estropea en cuanto se usa de verdad. | Lo encontró el propietario el 2026-09-13 y él mismo puso el control: con la gamma en 1,5 se veían cuadros alrededor de las letras de unos créditos, y en 1,0 dejan de verse. El mecanismo está medido en el código: `PictureAdjustment` construye una tabla de **256 niveles en 8 bits** y escribe `(byte)(256d * v)`, así que una curva pronunciada manda niveles de entrada distintos al mismo nivel de salida y un degradado se convierte en parches planos — banda de cuantización. El arreglo conocido es **difuminado** (añadir menos de un nivel de ruido ordenado antes de cuantizar), que es lo que AMD incluye como ayuda junto a su tono; la tabla tendría que salir en más precisión y cuantizar al escribir el píxel. **Y hace falta una puerta que lo vea**: hoy ninguna prueba mide bandas, y por eso esto llegó a producción y lo encontró una persona. Cuidado con el coste: ese bucle corre dos millones de veces por segundo. |
