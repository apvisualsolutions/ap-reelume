...
4. ~~**The scalars gate**~~ — **done on 2026-08-19**: [the evidence](evidence/stable/audit-scalar-gate.md). As decided, with two things the decision did not have: the `MotionDuration*` pair was watched by **two** tests, not one, and their guarantee **moved** onto `FluentThemeService.MotionDuration` rather than being lost. Proved failing in **three** directions. **The scalars gate counts consumption in ANY `.axaml` under `src/`**, the token file included — a
