# El póster llega a la ficha / The poster reaches the film card
...
owner's decision 14: the film card's header draws the real poster, with the generated art beneath it.
...
UiTests           1.002 superadas, 0 con error
...
| `MovieDetailsViewModel.cs` | 100 | 100 |
...
número del artefacto `coverage-debt` del run que lo mida. / Two files rise above their declared
...
tiempo y 455× los controles vivos por perder la virtualización. Esto es una superficie, no tres. Y la
ficha de serie dibuja su póster en el prototipo y aquí todavía no. / The library grid's covers and
Home's three rows are still out, for the measured reason of 2026-08-21. And the show card draws its
poster in the prototype and not yet here.
