...
- **Paneles «de cristal» (menús, diálogos, mini reproductor)** — `backdrop-filter: blur()` no tiene equivalente fuera del shell. Mica vive aislada en `MicaBackdropService` (host Windows) con fallback sólido; ningún paquete de *acrylic*/blur está referenciado. En AXAML son `SolidColorBrush` casi opacos (el token `--glass` ya lo es), sin desenfoque real detrás.
  - **Excepción medida el 2026-09-12: el distintivo de tipo de la portada SÍ lo lleva.** La línea de arriba sigue siendo cierta para un panel que flota sobre contenido cualquiera, y lo confirmó la fuente de Avalonia: no hay desenfoque de fondo dentro de la ventana —`BlurEffect` difumina la capa del propio elemento y el acrílico sólo tiñe, apoyado en la transparencia de la ventana—. Lo que cambia en el chip es que **lo que hay detrás lo dibuja la propia tarjeta**: pinta un pincel del arte de su portada, su propia caja recortada de él y repetida en espejo hacia fuera (el `edgeMode="mirror"` que pide la especificación de filtros), y lo difumina con el radio que Skia convierte en el sigma de los 8 px de CSS. Medido: la trama del arte pierde el 97,6 % de su detalle bajo el chip. Evidencia en `docs/evidence/stable/audit-poster-card-rhythm.md`.
