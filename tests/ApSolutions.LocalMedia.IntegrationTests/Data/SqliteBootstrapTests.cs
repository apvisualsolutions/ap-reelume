...
public sealed class SqliteBootstrapTests
{
    ...
    public async Task Bootstrap_uses_WAL_foreign_keys_busy_timeout_and_the_current_schema()
    {
        ...
        Assert.Equal(24L, await ScalarInt64Async(connection, "SELECT COUNT(*) FROM schema_history;"));
        Assert.Equal(24L, await ScalarInt64Async(connection, "SELECT MAX(version) FROM schema_history;"));
        Assert.Equal(
            "initial,library_roots,media_files_scans,catalog_fts,file_identity,scanned_catalog_projection,match_candidates,metadata_cache,rename_log,playback_preferences,watch_state,intro_markers,personal_state,episode_media,catalog_metadata_versions,detected_markers,trailer_key,provider_reference,match_candidate_title,five_star_rating,scanned_title_year,courses,picture_adjustment,personal_cover",
    ...
    public async Task Migration_is_idempotent_and_creates_one_valid_copy_per_new_migration()
    {
        ...
        Assert.Equal(24, backups.Length);
        ...
        Assert.Equal(24L, await ScalarInt64Async(active, "SELECT COUNT(*) FROM schema_history;"));
