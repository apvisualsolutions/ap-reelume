...
quien la usa. Sin cuentas, sin telemetría, sin servidor. Licencia propia, gratuita para quien la usa (`LicenseRef-APSolutions`).
...
1. [docs/FEATURES.md](docs/FEATURES.md) — el registro **canónico** del alcance: qué existe, en qué
   ...
   **Para «qué falta» no se lee a mano: `pwsh -NoProfile -File eng/list-pending.ps1`.** Son 75 <!--medido:identificadores-de-alcance-->
   filas
...
`UpdateMetadata.cs` llegó a 100/100 al ganar un campo con `LIB-021` —la previsualización lo leyó
subiendo y, en vez de copiar el suelo, se cubrió lo que faltaba—. Antes estuvo en **187** desde el 2026-09-13, y ese día se movió **dos veces**, que es lo que enseña cómo funciona. Primero
...
a 100/100 quitando tres guardas del clic que nada podía tomar. Y a 185 por la vía que se olvida:
pruebas nuevas recorrieron `AudioOutputViewModel.cs` de paso, CI lo midió mejor que su suelo —la
previsualización local no, porque esta máquina tiene audio y el runner no—, y se cubrieron las
cinco ramas que le faltaban. Estuvo en 189 desde el 2026-09-05, cuando subió por una vista
...
1. **Licencia por archivo.** Todo fuente nuevo lleva `SPDX-License-Identifier: LicenseRef-APSolutions`. Lo
